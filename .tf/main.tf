...
resource "aws_api_gateway_base_path_mapping" "yo_api_base_path" {
  for_each        = toset(var.root_domains)
  domain_name     = aws_api_gateway_domain_name.yo_api_domain[each.key].domain_name
...
resource "aws_route53_record" "yo_api_cname" {
  ...
  type     = "CNAME"
  ttl      = 300
  records  = [aws_api_gateway_domain_name.yo_api_domain[each.key].cloudfront_domain_name]
}
